...
public interface MemberSyncInfoApiDocs {
    ...
    @ApiResponses(
            value = {
                    @ApiResponse(responseCode = "200", description = "사용자 연동 정보 조회 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = MemberSyncInfoResult.class))),
                    @ApiResponse(responseCode = "400", description = "잘못된 요청", content = @Content),
                    @ApiResponse(responseCode = "500", description = "INTERNAL SERVER ERROR", content = @Content)
            }
    )
    ResponseEntity<SuccessResponse<MemberSyncInfoResult>> getMemberSyncInfo(
